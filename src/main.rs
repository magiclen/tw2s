...
fn main() -> anyhow::Result<()> {
    ...
    generate_static_dictionary(temporary_path.as_path(), DefaultConfig::TW2SP)
        .map_err(|error| anyhow!(error))
        .with_context(|| anyhow!("{temporary_path:?}"))?;

    let opencc = OpenCC::new(temporary_path.join(DefaultConfig::TW2SP))
        .with_context(|| anyhow!("{temporary_path:?}"))?;
        ...
        Some(tw_path) => {
            ...
            match s_path.metadata() {
                Ok(metadata) => {
                    ...
                    if tw_path_canonical == s_path_canonical {
                        return Err(anyhow!("{s_path:?} is the same file as {tw_path:?}!"));
                    }
                },
            ...
            let s_file = File::create(s_path.as_path()).with_context(|| anyhow!("{s_path:?}"))?;

            if let Err(error) =
                convert_file(&opencc, tw_file, tw_path.as_path(), s_file, s_path.as_path())
            {
...
fn convert_file(
    ...
    let mut s_file = BufWriter::new(s_file);
    ...
    loop {
        ...
        let c = tw_file.read_line(&mut line).with_context(|| anyhow!("{tw_path:?}"))?;
    ...
    s_file.flush().with_context(|| anyhow!("{s_path:?}"))
}
